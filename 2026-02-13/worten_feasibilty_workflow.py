...
for pdp_url in product_urls:
    ...
    try:
        ...
        json_data = json.loads(script_data) if script_data else {}
        data["brand"] = json_data.get("brand", {}).get("name")
        data["selling_price"] = json_data.get("offers", {}).get("price")
        data["regular_price"] = json_data.get("offers", {}).get("highPrice")
        data["rating"] = json_data.get("aggregateRating", {}).get("ratingValue")
        data["review"] = json_data.get("aggregateRating", {}).get("reviewCount")
        desc = selector.xpath("//div[contains(@class,'about')]//text()").getall()
        ...
        data["colour"] = selector.xpath("//*[contains(text(),'Cor')]/following::span[1]/text()").get()
...
print("DONE")
in this which is crawler and which is parser
